...
class ApiGatewayService extends MoleculerService {
    ...
    @Method
    // @ts-ignore 6133
    private async authorize(ctx: Context<{}, { user: Object; token: string, userID: string}>, route: Route, req: IncomingRequest, res: GatewayResponse): Promise<Context> {
        ...
        if (type !== "Token" && type !== "Bearer") {
            // Invalid auth type
            return Promise.reject(
                this.newResponse(new WebErrors.UnAuthorizedError(WebErrors.ERR_UNABLE_DECODE_PARAM, null))
            );
        }   
        ...
        if (req.$action.auth == "required" && !user) {
            return Promise.reject(
                new WebErrors.UnAuthorizedError("UNAUTHORIZED", "No user found")
            );
        }
